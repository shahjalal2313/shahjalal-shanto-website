...
const SocialIcon = ({ icon, className = '' }: SocialIconProps) => {
  ...
  return <svg className={className} fill="currentColor" viewBox="0 0 24 24">{icons[icon as keyof typeof icons]}</svg>;
};
...
export default function SocialLinks({ layout = 'horizontal', showDescriptions = false }: SocialLinksProps) {
...
const SocialLinkItem = ({ link, showDescription }: SocialLinkItemProps) => {
  if (showDescription) {
    return (
